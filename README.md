...
- 增加前端自动化测试，尤其是 mocked `window.Spotify` 的 SDK 状态流转测试。
- 优化播放进度条平滑度和 SDK/backend 状态 reconciliation。
- 继续提升推荐准确度，包括主题推荐、专辑边界、同名歌曲与歌手精确匹配。
- 增强 LLM rerank 的可解释性和可观测性。
- 从单 bridge account 迁移到多用户 Spotify OAuth 绑定。
- 将受控 planner 逐步升级为更完整的工具调用型音乐 Agent。
